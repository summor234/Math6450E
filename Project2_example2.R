#####Replicate the results from the article "A robust EM clustering algorithm for Gaussian Mixture Model" by Yang el at
library(MASS)
library(mvtnorm)
library(gdata)
library(ellipse)
...
covar_1 <- matrix(c(1.2,0,0,0.01), nrow = 2, byrow = TRUE)
covar_2 <- matrix(c(1.2,0,0,0.01), nrow = 2, byrow = TRUE)
covar_3 <- matrix(c(1.2,0,0,0.01), nrow = 2, byrow = TRUE)
covar_4 <- matrix(c(0.01,0,0,0.8), nrow = 2, byrow = TRUE)
...
for (z in 1:loop){
  ...
  distr_3 <- mvrnorm(n_3,mu=mu_3,Sigma=covar_3)
  distr_4 <- mvrnorm(n_4,mu=mu_4,Sigma=covar_4)
  
  X <- rbind(distr_1,distr_2,distr_3,distr_4)
...
mu_c <- mu_w[which(unlist(cluster)==4)]
covariance_c <- sigma_w[which(unlist(cluster)==4)]

mean <- c(rep(0,d))
covariance <- matrix(c(rep(0,d*d)), nrow = d, byrow = TRUE)

for (i in 1:length(unlist(cluster)[unlist(cluster)==4])){
  mean = mean + mu_c[[i]]/length(unlist(cluster)[unlist(cluster)==4])
  covariance = covariance + covariance_c[[i]]/length(unlist(cluster)[unlist(cluster)==4])
}

length(unlist(cluster)[unlist(cluster)==4])
...
lines(ellipse(x=EM_sigma[[1]][[3]], centre=EM_mu[[1]][[1]],level=0.95)[,1],ellipse(x=EM_sigma[[1]][[3]], centre=EM_mu[[1]][[3]],level=0.95)[,2],type = "l",col="blue")
lines(ellipse(x=EM_sigma[[1]][[4]], centre=EM_mu[[1]][[2]],level=0.95)[,1],ellipse(x=EM_sigma[[1]][[4]], centre=EM_mu[[1]][[4]],level=0.95)[,2],type = "l",col="blue")
